...
public class PostServiceImpl implements IPostService{
    ...
    @Override
    public Page<Post> getPostsCategory(Integer categoryId, int page, int limit) {
        PageRequest pageRequest=PageRequest.of(page-1,limit);
        return postRepository.findAllByCategoryId(categoryId,pageRequest);
    }

    @Override
    public Page<Post> getPostsAuthor(Integer authorId, int page, int limit) {
        PageRequest pageRequest=PageRequest.of(page-1,limit);
        return postRepository.findAllByAuthorId(authorId,pageRequest);
    }

    @Override
    public Post getPostSLug(String slug) {
    ...
    @Override
    public Page<Post> getAllPost(int page, int limit) {
        PageRequest pageRequest=PageRequest.of(page-1,limit);
    ...
    @Override
    public Page<Post> findByStatus(String status, int page, int limit) {
        PageRequest pageRequest=PageRequest.of(page-1,limit);
        return postRepository.findAllByStatus(status,pageRequest);
    }

    @Override
    public Page<Post> searchPosts(String keyword, int page, int limit) {
        PageRequest pageRequest=PageRequest.of(page-1,limit);
